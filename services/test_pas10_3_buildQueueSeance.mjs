...
// ─── Fix Anglais 25 — mode relecture (cf. BIBLE §11.4) ─────────────────

group("Mode relecture");

test("relecture : coldLesson seule (chutes/nouveaux/j1 vides) → cold_lesson + exos lourds régénérés", () => {
  // Scénario : l'enfant a fini sa séance, les acquiredToday ont été
  // placés dans coldLesson par le composer. ExerciseService doit régénérer
  // les exos lourds pour ces mots, pour que l'enfant puisse les refaire.
  const seance = {
    type: "normale",
    coldLesson: [1, 2],
    chutes: [],
  ...
  const repo = makeRepo([apple, banana]);
  const q = service.buildQueueSeance(seance, repo);

  // Doit contenir : 2 cold_lesson + exos lourds pour 2 mots
  const coldCount = q.filter(e => e.mode === "cold_lesson").length;
  eq(coldCount, 2, "Doit contenir 2 cold_lesson");
  const nonCold = q.filter(e => e.mode !== "cold_lesson");
  assert(nonCold.length > 0, "Les exos lourds doivent être régénérés pour la relecture");
});

test("relecture : si j1 présent, on ne déclenche pas le mode relecture", () => {
  // Sécurité : si j1 est non vide, ce n'est pas une "séance terminée",
  // c'est une séance en cours avec des J+1 à faire → comportement normal.
  const seance = {
    type: "normale",
    coldLesson: [1],
    chutes: [],
  ...
  // 1 cold_lesson (apple) + 0 exos lourds (chutes/nouveaux vides, j1 non vide
  // → pas de relecture déclenchée) + N exos j1 pour banana
  const coldCount = q.filter(e => e.mode === "cold_lesson").length;
  eq(coldCount, 1);
  // Pas d'exos pour apple (id 1) en dehors de cold_lesson : c'est la
  // sémantique normale, pas la relecture.
  const exosPourApple = q.filter(e => e.mode !== "cold_lesson" && e.word?.id === 1);
  eq(exosPourApple.length, 0, "Apple n'a pas d'exos lourds (pas en mode relecture)");
});
